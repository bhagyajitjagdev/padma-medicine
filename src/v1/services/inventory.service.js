...
exports.checkInventory = async (batchCode) => db.Inventory.findOne({ batchCode, isDeleted: false });

exports.createInventory = async (body) => db.Inventory.create(body);

exports.getInventoryByKey = async (body) =>
  db.Inventory.findOne({ ...body, isDeleted: false, quantity: { $gt: 0 } }).lean();

exports.getInventories = async (body, { page = 1, limit = 10, order = -1 } = {}) => {
  const inventories = await db.Inventory.find({ ...body, isDeleted: false, quantity: { $gt: 0 } })
